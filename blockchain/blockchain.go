...
// snapshotDisco representa a estrutura exata que será salva no arquivo JSON.
// Contém a cadeia de blocos e a "fotografia" atual dos saldos das companhias.
type snapshotDisco struct {
    Blocos []Bloco        `json:"blocos"`
    Saldos map[string]int `json:"saldos"`
}

// caminhoArquivo resolve o diretório de estado baseado nas variáveis de ambiente
// e retorna o caminho completo do arquivo da chain para este broker específico.
...
// SalvarChain persiste a blockchain inteira no disco de forma segura.
// Utiliza um padrão de "Escrita Atômica" (escreve em um .tmp e depois renomeia)
// para evitar que o arquivo corrompa se o container cair no meio da gravação.
func (c *Chain) SalvarChain() error {
    ...
    // Copia os dados de forma segura sob o lock de leitura
    ...
    // 🛡️ Garante que a árvore de diretórios base existe antes de gravar
    if err := os.MkdirAll(filepath.Dir(caminho), 0755); err != nil {
        return fmt.Errorf("erro ao criar diretório de estado: %w", err)
    }

    // Escrita Atômica: Escreve no temporário e depois substitui o oficial.
    // Isso garante que o arquivo chain_X.json nunca fique pela metade.
    tmp := caminho + ".tmp"
...
// CarregarChain lê o arquivo JSON do disco e o converte para a estrutura snapshotDisco.
// Retorna (nil, nil) se for a primeira execução do broker (arquivo não existe).
...
// RestaurarChain recebe um snapshot carregado do disco, remonta a estrutura da Chain
// em memória e faz uma validação criptográfica completa antes de aceitá-la.
func RestaurarChain(brokerID string, snapshot *snapshotDisco) *Chain {
    ...
    // Se o arquivo foi adulterado manualmente com um bloco falso, a chain é descartada
    if !c.ValidarChain() {
...
// Bloco representa uma unidade imutável de informação no ledger.
type Bloco struct {
...
// Chain encapsula a cadeia de blocos, o livro-razão de saldos e os mecanismos
// de concorrência (RWMutex) para acessos assíncronos seguros.
type Chain struct {
    mu      sync.RWMutex
    Blocos  []Bloco
    Saldos  map[string]int
    DroneID string // Atua como Identificador do Validador Local
}
...
// NovaChain é o construtor principal. Tenta carregar o estado salvo no disco.
// Caso não exista, ele minera o Bloco Gênesis e estabelece os fundos iniciais da rede.
func NovaChain(brokerID string, saldosIniciais map[string]int) *Chain {
    ...
    // Se falhou ao restaurar ou não existe, cria do zero
    c := &Chain{
    ...
    // Data fixa para garantir que o Hash do Gênesis seja idêntico em todos os nós
    ...
    // Persiste o estado inicial recém-criado
...
// CalcularHash converte todos os campos de um Bloco em uma única string
// e aplica o algoritmo SHA-256 para gerar a assinatura digital do bloco.
...
// ProporBloco gera um novo candidato a bloco apontando para o último bloco atual.
// Retorna a estrutura pronta, mas NÃO a insere na chain local (depende de consenso).
func (c *Chain) ProporBloco(tipoDados TipoDados, dados interface{}, validador string) (Bloco, error) {
    ...
    // Lê a ponta atual da chain para vincular o novo bloco
    c.mu.RLock()
...
// CommitarBloco anexa definitivamente um bloco aprovado pelo consenso na chain.
// Verifica rigorosamente se o bloco ainda é válido para o estado atual da rede.
func (c *Chain) CommitarBloco(b Bloco) error {
    c.mu.Lock()

    ultimo := c.Blocos[len(c.Blocos)-1]

    // Defesas contra inserções fora de ordem ou corrompidas
    if b.Indice != ultimo.Indice+1 {
        c.mu.Unlock() // Evita deadlock em caso de erro
    ...
    // Agora é seguro chamar SalvarChain (que exige obter o lock interno)
    if err := c.SalvarChain(); err != nil {
...
// ConsultarSaldo retorna os fundos atuais de uma companhia.
...
// ValidarTransacao verifica antecipadamente se uma companhia possui fundos.
// Executada antes do processo de consenso para evitar trabalho desnecessário.
...
// DebitarCreditos subtrai o valor da conta. Usado quando o bloco de despacho é commitado.
...
// CreditarSaldo adiciona fundos à conta. Usado para recompensas (Ricart-Agrawala) e Recargas.
...
// ValidarChain varre todos os blocos garantindo que as assinaturas (Hashes) se conectam
// de ponta a ponta. Crucial para barrar históricos manipulados.
func (c *Chain) ValidarChain() bool {
    c.mu.RLock()
    defer c.mu.RUnlock()

    // Começa do índice 1 para não comparar o Gênesis com o vazio
...
// SubstituirChain resolve Conflitos de Rede (Forks) ou Sincroniza nós defasados.
// Se aceita, ela substitui a chain local e reconstrói completamente o estado financeiro (Saldos).
func (c *Chain) SubstituirChain(nova []Bloco, saldosIniciais map[string]int) bool {
    c.mu.Lock()
    defer c.mu.Unlock()

    // 🛡️ Previne que nós injetem chains mais curtas (regressão maliciosa)
    if len(nova) < len(c.Blocos) {
        return false
    }

    // ⚔️ TIE-BREAKER (Desempate em Fork Simutâneo)
    // Quando dois brokers criam um bloco de tamanho igual no mesmo momento, a rede trava.
    // Usamos o menor Hash Criptográfico como árbitro neutro. O menor vence, unificando a rede.
    if len(nova) == len(c.Blocos) {
        ultimoLocal := c.Blocos[len(c.Blocos)-1]
        ultimoRemoto := nova[len(nova)-1]

        if ultimoLocal.Hash <= ultimoRemoto.Hash {
            return false // Nosso Hash é menor ou igual, então nossa Chain Local vence a disputa.
        }
    }

    // Validação de integridade da nova Chain inteira
    ...
    // Chain aceita. Substitui o histórico local.
    c.Blocos = nova

    // 🧹 Limpeza completa do mapa de saldos antes de refazer o estado financeiro
    c.Saldos = make(map[string]int)
    for k, v := range saldosIniciais {
        c.Saldos[k] = v
    }

    // Replay financeiro: Repassa todas as transações válidas do bloco Gênesis até a ponta
    // Isso garante que os saldos refletem perfeitamente o novo estado de Consenso.
    ...
    // Salva o novo estado em disco assincronamente para não prender o Lock atual
    go func() {
        if err := c.SalvarChain(); err != nil {
...
// SerializarChain exporta toda a cadeia para JSON (usado para responder requisições na rede).
...
// SerializarSaldos exporta o estado financeiro para JSON (usado para diagnóstico/APIs).
...
// Tamanho retorna a altura (quantidade de blocos) da blockchain local.
...
// UltimoBloco retorna uma cópia do bloco na ponta da cadeia (usado para checar Hashes).
...
// Fundamental para auditoria de histórico (como defender a rede contra o Ataque Salami)
// permitindo varreduras sem deixar a chain travada (evita deadlocks prolongados).
